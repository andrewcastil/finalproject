...
The above plot visualizes mean score for each item grouped by factor. This plot provides an assurance that each item is an appropriate measure by demonstrating a similarity in mean response for items across each factor. If there were a deviation from a uniform distribution in this plot, it would be an indication that (a) certain item(s) should not be used. This plot shows that, on average, individuals tend to agree a bit more with resentment-related statements and disagree a bit more with hopeless and shame-related statements.
